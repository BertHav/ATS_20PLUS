...
// Modified by Bert Havinga
// january 2026
// https//github.com/BertHav/ATS_20PLUS
// main differences
// This implementation uses a 24c256 EEPROM containing the SSB patch, leaving memory space for improvements 
// Stereo indicator added
// RDS implementation changed, based on PU2CLR but slightly modified
// ----------------------------------------------------------------------
...
void setup() {
  ...
  if (!(PINC & (1 << (ENCODER_BUTTON - 14)))) {
    ...
    oledPrint("ATS_EX v1.19BH", 8, 2);
    oledPrint("Goshante 2024", 12, 4);
    oledPrint("Berts firmware", 8, 6);
    delay(2000);
  }
  oled.clear();
...
void showProgramInfo() {
  ...
  if (programInfo == NULL || (millis() < delayProgramInfo)) {
    return;
  }
  cleanBfoRdsInfo();
  programInfo[61] = '\0';  // Truncate the message to fit on display line
  strncpy(txtAux, &programInfo[progInfoIdx], sizeof(txtAux)-1);
 // Serial.println((char*)programInfo);
  txtAux[16] = '\0';
  progInfoIdx += 2;
//  if (progInfoIdx > (60 - sizeof(txtAux)) || (progInfoIdx > (sizeof(txtAux)+2)) || (programInfo[0] != prevchar)) {
  if (progInfoIdx > (16 + sizeof(txtAux)) || (programInfo[0] != prevchar)) {
    prevchar = programInfo[0];
    progInfoIdx = 0;
  }
  oled.setCursor(0, 6);
  oled.print(txtAux);
...
//    g_rdsActiveInfo = 0;
  ...
  {
    if (g_si4735.getRdsSync() && g_si4735.getNumRdsFifoUsed() > 0)
    {
        ...
        if (bShowStationName) { // time to show Station Name 
           stationName = g_si4735.getRdsStationName();
           showStationName();
        } else { // time to show Program Information
          programInfo = g_si4735.getRdsProgramInformation();
          showProgramInfo();
        }
    }
  }
...
*/
void showStereo() {
  if ((g_currentMode == FM) && !g_settingsActive) {
    oled.setCursor(22, 0);
    oled.print(" ");
    oled.setCursor(22, 0);
    if (g_si4735.getCurrentPilot()) {
      oled.invertOutput(true);
      oled.print("s");
    }
    oled.invertOutput(false);
...
void loadSSBPatch() {
  ...
  delay(50);
#ifdef SSB_PATCH_FROM_EEPROM
  oledPrint(_literal_EmptyLine, 0, 6, DEFAULT_FONT);
  eep = g_si4735.downloadPatchFromEeprom(eeprom_i2c_address);
  oledPrint("SSB patch", 8, 6);
  oledPrint(eep.refined.patch_id[0] == 's' ? "OK" : "FAIL", 88, 6);
  delay(1000);
  if (eep.refined.patch_id[0] != 's') {
    oledPrint((char*)eep.refined.patch_id, 0, 6);
    delay(2000);
  }
...
void loop() {
  ...
  if (millis() - g_lastFreqChange >= 1000) {
    ...
    if ((millis() - elapsedRSSI) > MIN_ELAPSED_RSSI_TIME * 9) {
      g_si4735.getCurrentReceivedSignalQuality();
      int aux = g_si4735.getCurrentRSSI();
      if (rssi != aux) {
        rssi = aux;
        showStereo();
      }
